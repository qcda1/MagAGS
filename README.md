...
*I believe a 4024 with ME-AGS-N would work as well.*

## Python
The provided code will allow you to manage the USB relay board from the Raspberry Pi. You will be able to adapt the code to your own environment. In my case for example, I have several Python programs that acquire operational data, store the information in database file and report with a Web user interface. You might have different equipment that require specific programs adapted to their own communication interfaces and protocol.
...
3. Plug your USB dual relay into the Raspberry Pi
4. Run relay.py You should hear and see the relay being switched on and off
5. If all good, you can run manage_gen.py as is by default. I will simulate the operation
...
the SOC. I estimated the SOC from the Classic 150 with Whizbang JR is the most accurate so that will be
...
As a reference, here is an excerpt of the [ME-AGS-N manual](Excerpt-ME-AGS-N-manual.md) and the [ME-PT2 Instruction Sheet](ME-PT2-Instruction-Sheet.md)

## Application
Included in this repository is a copy of my automatic generator start program MagAGS.py. It is made of a never-ending loop that monitors the battery bank's State Of Charge (SOC) and drive the generator start relay based on a lower and upper limit of the SOC. The system parameters can be adjusted in the yaml file MagAGS.yaml.
The SOC is read from an operational data acquisition database that keeps track of my solar energy system components. The program can be easily adapted to any systems that tracks a battery SOC. Note also that the program has a generator exercise function. Simply provide the time and duration and the generater will run under these on every first sunday of each month. This is the minimum my Kohler 14RCA require in the summer when not in use.
